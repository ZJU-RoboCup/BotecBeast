...
bool DynamixelDriver::getRawBulkReadData(uint8_t *id, uint8_t id_num, uint16_t *address, uint16_t *length, int32_t *data, const char **log)
{
  /* 所有读取数据存入数组 */
  uint32_t temp=0;
  int32_t  tempArray[240];
  uint8_t  index = 0;
  ErrorFromSDK sdk_error = {0, false, false, 0};
  ...
  for (int i = 0; i < id_num; i++)
  {
    ...
    {
      temp = groupBulkRead_->getData(id[i], 
                                     address[i], 
                                     length[i],
                                     tempArray);
      for(int j =0; j < length[i]; j++)
      {
        data[index] = tempArray[j];
        index++;
      }
    }
  }
