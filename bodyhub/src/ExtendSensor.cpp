...
bool SensorWrite(uint8_t WriteCount, uint8_t *WriteID, uint16_t *WriteAddress,
  ...
  for (uint8_t index = 0; index < WriteCount; index++) {
    result = dxl_wb.writeRegister(WriteID[index], WriteAddress[index],
...
void SensorbulkReadPoll() {
  if (SensorsList.size()) {
    ...
    uint8_t dataIndex = 0;
    dxlBulkRead(bulkReadID, readCount, bulkReadAddress, bulkReadLength,
                bulkReadData);
...
bool SensorPing(std::string name) {
  bool result = false;
  const char *log = NULL;
  result = dxl_wb.ping(SensorNameIDMap.at(name), &log);
  if (result == false) {
...
#if DEGUG_EN
  ROS_INFO("name: %s, id: %d, addr: %d, paramLen: %d", msg->SensorName.c_str(),
           ControlParamete.id, ControlParamete.addr, ControlParamete.lenght);
#endif
...
void sensorThread() {
  ...
  ROS_INFO("Start 'sensorThread' thread...");
  ExtendSensorInit();
...
void sensorTimerThread() {
  if (SimControll::simEnable) return;
  ROS_INFO("Start 'sensorTimerThread' thread...");
