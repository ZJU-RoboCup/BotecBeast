 ...
 * Hold joint name (used as identifier) and compute transformation, speed and
 * motion subspace. WARNING : Free joint translation parameters are in FP
 * coordinate (Example 4.5 p81).
 */
class Joint {
 public:
  /// Joint type.
 ...
 public:
  ...
  bool forward() const { return dir_ == 1.; }
   ...
   */
  int params() const { return params_; }
  ...
  bool isMimic() const { return isMimic_; }

  /// @return Mimiced name
  const std::string& mimicName() const { return mimicName_; }

  /// @return Mimic multiplier
  double mimicMultiplier() const { return mimicMultiplier_; }

  /// @return Mimic offset
  double mimicOffset() const { return mimicOffset_; }
   ...
   * @see tanAccel
   * @throw std::domain_error If the number of generalized acceleration variable
   * is wrong.
   */
  sva::MotionVecd sTanAccel(const std::vector<double>& alphaD) const;

  bool operator==(const Joint& b) const { return name_ == b.name_; }
...
inline Joint::Joint(Type type, const Eigen::Vector3d& axis, bool forward,
                    std::string name)
    : dir_(forward ? 1. : -1), params_(0), dof_(0), name_(name) {
...
inline Joint::Joint(std::string name)
    : dir_(1), params_(0), dof_(0), name_(name) {
  constructJoint(Joint::Type::Fixed, Eigen::Vector3d::UnitZ());
}

inline void Joint::makeMimic(const std::string& name, double multiplier,
                             double offset) {
  isMimic_ = true;
  mimicName_ = name;
...
inline sva::PTransform<T> Joint::pose(const std::vector<T>& q) const {
  ...
  switch (type_) {
    case Rev:
      ...
      return PTransform<T>(
          AngleAxis<T>(-q[0], S_.block<3, 1>(0, 0).cast<T>()).matrix());
    case Prism:
      return PTransform<T>(Vector3<T>(S_.block<3, 1>(3, 0).cast<T>() * q[0]));
    case Spherical:
      return PTransform<T>(
          Quaternion<T>(q[0], dir_ * q[1], dir_ * q[2], dir_ * q[3]).inverse());
    case Planar:
      rot = sva::RotZ(q[0]);
      if (dir_ == 1.) {
    ...
    case Cylindrical:
      return PTransform<T>(
          AngleAxis<T>(-q[0], S_.col(0).head<3>().cast<T>()).matrix(),
...
inline sva::MotionVecd Joint::motion(const std::vector<double>& alpha) const {
  ...
  switch (type_) {
    case Rev:
      return MotionVecd(
          (Vector6d() << S_.block<3, 1>(0, 0) * alpha[0], Vector3d::Zero())
              .finished());
    case Prism:
      return MotionVecd(
          (Vector6d() << Vector3d::Zero(), S_.block<3, 1>(3, 0) * alpha[0])
              .finished());
    case Spherical:
    ...
    case Free:
      return MotionVecd(S_ *
                        (Vector6d() << alpha[0], alpha[1], alpha[2], alpha[3],
                         alpha[4], alpha[5])
                            .finished());
    case Fixed:
    default:
...
inline sva::MotionVecd Joint::tanAccel(
    const std::vector<double>& alphaD) const {
  using namespace Eigen;
  using namespace sva;
  switch (type_) {
    case Rev:
      return MotionVecd(
          (Vector6d() << S_.block<3, 1>(0, 0) * alphaD[0], Vector3d::Zero())
              .finished());
    case Prism:
      return MotionVecd(
          (Vector6d() << Vector3d::Zero(), S_.block<3, 1>(3, 0) * alphaD[0])
              .finished());
    case Spherical:
    ...
    case Free:
      return MotionVecd(S_ *
                        (Vector6d() << alphaD[0], alphaD[1], alphaD[2],
                         alphaD[3], alphaD[4], alphaD[5])
                            .finished());
    case Fixed:
    default:
      return MotionVecd(Vector6d::Zero());
...
inline std::vector<double> Joint::zeroDof() const { return ZeroDof(type_); }

inline sva::PTransformd Joint::sPose(const std::vector<double>& q) const {
  if (q.size() != static_cast<unsigned int>(params_)) {
    std::ostringstream str;
    str << "Wrong number of generalized position variable: expected " << params_
        << " gived " << q.size();
...
inline sva::MotionVecd Joint::sMotion(const std::vector<double>& alpha) const {
  if (alpha.size() != static_cast<unsigned int>(dof_)) {
    std::ostringstream str;
    str << "Wrong number of generalized speed variable: expected " << params_
        << " gived " << alpha.size();
    throw std::domain_error(str.str());
  }
  return motion(alpha);
...
inline sva::MotionVecd Joint::sTanAccel(
    const std::vector<double>& alphaD) const {
...
inline Eigen::Matrix3<T> QuatToE(const std::vector<T>& q) {
  ...
  return 2. *
         (Matrix3<T>() << p0s + p1s - 0.5, p1p2 + p0p3, p1p3 - p0p2,
          p1p2 - p0p3, p0s + p2s - 0.5, p2p3 + p0p1, p1p3 + p0p2, p2p3 - p0p1,
          p0s + p3s - 0.5)
             .finished();
}
