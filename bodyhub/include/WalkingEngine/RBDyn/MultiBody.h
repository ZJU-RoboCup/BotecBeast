...
class MultiBody {
 public:
  ...
  int nrBodies() const { return static_cast<int>(bodies_.size()); }

  /// @return Number of joints.
  int nrJoints() const { return static_cast<int>(joints_.size()); }

  /// @return Bodies of the multibody system.
  const std::vector<Body>& bodies() const { return bodies_; }

  /// Set bodies of the multibody system.
  void bodies(std::vector<Body> b) { bodies_ = std::move(b); }

  /// @return Body at num position in bodies list.
  const Body& body(int num) const { return bodies_[num]; }

  /// Set the body num in bodies list.
  void body(int num, const Body& b) { bodies_[num] = b; }
  ...
  const Joint& joint(int num) const { return joints_[num]; }

  /// @return Predeccesor body index of each joint.
  const std::vector<int>& predecessors() const { return pred_; }

  /// @return Predecessor body of joint num.
  int predecessor(int num) const { return pred_[num]; }

  /// @return Successor body index of each joint.
  const std::vector<int>& successors() const { return succ_; }

  /// @return Successor body of joint num.
  int successor(int num) const { return succ_[num]; }

  /// @return Parent body index of each body.
  const std::vector<int>& parents() const { return parent_; }

  /// @return Parent body of body num.
  int parent(int num) const { return parent_[num]; }

  /// @return Transformation from the body base to joint i
  const std::vector<sva::PTransformd>& transforms() const { return Xt_; }

  /// @return Set Transformation from the body base to joint i
  void transforms(std::vector<sva::PTransformd> Xt) { Xt_ = std::move(Xt); }

  /// @return Transformation from the body base to joint num
  const sva::PTransformd& transform(int num) const { return Xt_[num]; }

  /// Set the transformation from the body base to joint num
  void transform(int num, const sva::PTransformd& Xt) { Xt_[num] = Xt; }
  ...
  const std::vector<int>& jointsPosInParam() const { return jointPosInParam_; }

  /// @return the joint position in dof vector (alpha, alphaD…).
  const std::vector<int>& jointsPosInDof() const { return jointPosInDof_; }
  ...
  void sBodies(std::vector<Body> b) {
    if (b.size() != bodies_.size()) {
      ...
      str << "bodies vector size mismatch: expected size " << bodies_.size()
          << " gived " << b.size();
   ...
   */
  const Body& sBody(int num) const { return bodies_.at(num); }

  /** Safe version of @see body.
   * @throw std::out_of_range.
   */
  void sBody(int num, const Body& b) { bodies_.at(num) = b; }

  /** Safe version of @see joint.
   * @throw std::out_of_range.
   */
  const Joint& sJoint(int num) const { return joints_.at(num); }

  /** Safe version of @see predecessor.
   * @throw std::out_of_range.
   */
  int sPredecessor(int num) const { return pred_.at(num); }

  /** Safe version of @see successor.
   * @throw std::out_of_range.
   */
  int sSuccessor(int num) const { return succ_.at(num); }

  /** Safe version of @see parent.
   * @throw std::out_of_range.
   */
  int sParent(int num) const { return parent_.at(num); }
  ...
  void sTransforms(std::vector<sva::PTransformd> Xt) {
    if (Xt.size() != Xt_.size()) {
      std::ostringstream str;
      str << "transform vector size mismatch: expected size " << Xt_.size()
          << " gived " << Xt.size();
      throw std::runtime_error(str.str());
    }
    transforms(Xt);
   ...
   * @throw std::out_of_range.
   */
  const sva::PTransformd& sTransform(int num) const { return Xt_.at(num); }

  /** Safe version of @see transform.
   * @throw std::out_of_range.
   */
  void sTransform(int num, const sva::PTransformd& Xt) { Xt_.at(num) = Xt; }

  /** Safe version of @see jointPosInParam.
   * @throw std::out_of_range.
   */
  int sJointPosInParam(int i) const { return jointPosInParam_.at(i); }

  /** Safe version of @see jointPosInDof.
   * @throw std::out_of_range.
   */
  int sJointPosInDof(int i) const { return jointPosInDof_.at(i); }
