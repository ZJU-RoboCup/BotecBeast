...
#define LFOOT_FSR_THRESHOLD 70
#define RFOOT_FSR_THRESHOLD 70
...
class LIPMWalk {
 public:
  LIPMWalk(uint8_t mode = 0);
  ~LIPMWalk();

  void setRunMode(uint8_t mode = 0);
  void setEndEffector(std::string leftStr, std::string rightStr);

  void stablizer(void);
  void slopeStand(void);
  void setStepTime(double_t t);
  ...
  ParameterOfPosture getPostureInTorso(Eigen::VectorXd jointV);
  Eigen::VectorXd postureToJoint(ParameterOfPosture posture);
  std::vector<Eigen::VectorXd> initSquat(Eigen::VectorXd currentJointV);
  ...
  int squatStep = 80;
  double squatHeight;
  int endEffectorOfLeft;
  int endEffectorOfRight;
  ...
  Eigen::Vector2d observerMeasurementDeviation = {
      2, 2}; /**< The measurement uncertainty of the computed "measured" center
                of mass position */
  ...
  void zAdjustForHumanLike(double stepLen);
  ...
  Eigen::Vector6d cubicsplinePx_wF, cubicsplineAx_wF, cubicsplinePy_wF,
      cubicsplineAy_wF, cubicsplinePyaw_wF,
      cubicsplineAyaw_wF;  // world frame spline
  ...
  Eigen::Vector6d cubicsplineFootPx_wf, cubicsplineFootAx_wf,
      ...
      cubicsplineFootPpitch1_wf, cubicsplineFootApitch1_wf,
      cubicsplineFootPpitch2_wf, cubicsplineFootApitch2_wf,
      cubicsplineFootPpitch3_wf, cubicsplineFootApitch3_wf,
