...
/**
 * Compute the 3D rotation error between two matrix E_a_b and E_a_c in the 'a'
 * frame. This method convert the 3D rotation matrix E_b_c into a rotation
 * vector. The matrix E_b_c is computed as follow E_a_c = E_b_c*E_a_b. Then the
 * error is computed with E_b_a*rotationVelocity(E_b_c).
 ...
 */
template <typename T>
MotionVec<T> transformError(const PTransform<T>& X_a_b,
                            const PTransform<T>& X_a_c, double prec = 1e-8);
...
class PTransform {
 ...
 public:
  ...
  PTransform(const quaternion_t& rot)
      : E_(rot.matrix()), r_(vector3_t::Zero()) {}
  ...
  const matrix3_t& rotation() const { return E_; }

  /// @return Rotation matrix.
  matrix3_t& rotation() { return E_; }

  /// @return Translation vector.
  const vector3_t& translation() const { return r_; }

  /// @return Translation vector.
  vector3_t& translation() { return r_; }
  ...
  /// @return compute angular part of @see operator*(const MotionVec<T>& mv)
  /// const
  Eigen::Vector3<T> angularMul(const MotionVec<T>& mv) const;
  /// @return compute linear part of @see operator*(const MotionVec<T>& mv)
  /// const
  ...
  PTransform<T> inv() const { return PTransform<T>(E_.transpose(), -E_ * r_); }
...
template <typename T>
inline Vector3<T> rotationError(const Matrix3<T>& E_a_b,
                                const Matrix3<T>& E_a_c, double prec) {
...
inline Vector3<T> rotationVelocity(const Matrix3<T>& E_a_b, double /* prec */) {
  ...
  w = Vector3<T>(-E_a_b(2, 1) + E_a_b(1, 2), -E_a_b(0, 2) + E_a_b(2, 0),
                 -E_a_b(1, 0) + E_a_b(0, 1));
...
inline MotionVec<T> transformError(const PTransform<T>& X_a_b,
  ...
  return PTransform<T>(Matrix3<T>(X_a_b.rotation().transpose())) *
         transformVelocity(X_b_c, prec);
}
...
template <typename T>
PTransform<T> interpolate(const PTransform<T>& from, const PTransform<T>& to,
                          double t) {
  Eigen::Quaternion<T> qfrom(from.rotation());
  Eigen::Quaternion<T> qto(to.rotation());
  PTransform<T> result(qfrom.slerp(t, qto),
