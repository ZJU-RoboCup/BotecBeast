...
template <typename T>
Matrix3<T> inertiaToOrigin(const Matrix3<T>& inertia, T mass,
                           const Vector3<T>& com, const Matrix3<T>& rotation);
...
class RBInertia {
 ...
 public:
  ...
  // Accessor
  /// @return Mass.
  T mass() const { return m_; }

  /// @return Spatial momentum.
  const vector3_t& momentum() const { return h_; }

  /// @return Inertia matrix with a zero upper part.
  const matrix3_t& lowerTriangularInertia() const { return I_; }
  ...
  RBInertia<T2> cast() const {
    return RBInertia<T2>(T2(m_), h_.template cast<T2>(),
                         I_.template cast<T2>());
  }
  ...
  RBInertia<T> operator-() const { return RBInertia<T>(-m_, -h_, -I_); }
...
Matrix3<T> inertiaToOrigin(const Matrix3<T>& inertia, T mass,
                           const Vector3<T>& com, const Matrix3<T>& rotation) {
  Matrix3<T> trans = vector3ToCrossMatrix<T>(mass * com) *
                     vector3ToCrossMatrix<T>(com).transpose();
