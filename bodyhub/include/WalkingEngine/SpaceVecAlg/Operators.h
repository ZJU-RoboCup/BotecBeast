...
inline void colwiseCrossEq(const Eigen::MatrixBase<Derived1>& m1,
  ...
  Eigen::MatrixBase<Derived3>& result_nc =
      const_cast<Eigen::MatrixBase<Derived3>&>(result);
...
inline Eigen::Block<Derived, 3, Dynamic> motionAngular(
    Eigen::MatrixBase<Derived>& mv) {
  return Eigen::Block<Derived, 3, Dynamic>(mv.derived(), 0, 0, 3, mv.cols());
}

template <typename Derived>
inline Eigen::Block<const Derived, 3, Dynamic> motionAngular(
    const Eigen::MatrixBase<Derived>& mv) {
  return Eigen::Block<const Derived, 3, Dynamic>(mv.derived(), 0, 0, 3,
                                                 mv.cols());
}

template <typename Derived>
inline Eigen::Block<Derived, 3, Dynamic> motionLinear(
    Eigen::MatrixBase<Derived>& mv) {
  return Eigen::Block<Derived, 3, Dynamic>(mv.derived(), 3, 0, 3, mv.cols());
}

template <typename Derived>
inline Eigen::Block<const Derived, 3, Dynamic> motionLinear(
    const Eigen::MatrixBase<Derived>& mv) {
  return Eigen::Block<const Derived, 3, Dynamic>(mv.derived(), 3, 0, 3,
                                                 mv.cols());
}

template <typename Derived>
inline Eigen::Block<Derived, 3, Dynamic> forceCouple(
    Eigen::MatrixBase<Derived>& mv) {
  return Eigen::Block<Derived, 3, Dynamic>(mv.derived(), 0, 0, 3, mv.cols());
}

template <typename Derived>
inline Eigen::Block<const Derived, 3, Dynamic> forceCouple(
    const Eigen::MatrixBase<Derived>& mv) {
  return Eigen::Block<const Derived, 3, Dynamic>(mv.derived(), 0, 0, 3,
                                                 mv.cols());
}
...
template <typename Derived>
inline Eigen::Block<Derived, 3, Dynamic> forceForce(
    Eigen::MatrixBase<Derived>& mv) {
  return Eigen::Block<Derived, 3, Dynamic>(mv.derived(), 3, 0, 3, mv.cols());
}

template <typename Derived>
inline Eigen::Block<const Derived, 3, Dynamic> forceForce(
    const Eigen::MatrixBase<Derived>& mv) {
  return Eigen::Block<const Derived, 3, Dynamic>(mv.derived(), 3, 0, 3,
                                                 mv.cols());
}
...
inline MotionVec<T> MotionVec<T>::cross(const MotionVec<T>& mv2) const {
  return MotionVec<T>(
      angular().cross(mv2.angular()),
      angular().cross(mv2.linear()) + linear().cross(mv2.angular()));
}
...
inline ForceVec<T> MotionVec<T>::crossDual(const ForceVec<T>& fv2) const {
  return ForceVec<T>(
      angular().cross(fv2.couple()) + linear().cross(fv2.force()),
...
inline ABInertia<T> ABInertia<T>::operator+(const RBInertia<T>& rbI) const {
  ...
  Matrix3<T> M, I;
  M.template triangularView<Lower>() =
      massMatrix() + Matrix3<T>::Identity() * rbI.mass();
...
inline ForceVec<T> ABInertia<T>::operator*(const MotionVec<T>& mv) const {
  return ForceVec<T>(
      inertia() * mv.angular() + gInertia() * mv.linear(),
...
template <typename T>
inline Eigen::Vector3<T> PTransform<T>::angularMul(
    const MotionVec<T>& mv) const {
  using namespace Eigen;
...
template <typename T>
inline Eigen::Vector3<T> PTransform<T>::linearMul(
    const MotionVec<T>& mv) const {
  using namespace Eigen;
  const Matrix3<T>& E = rotation();
...
template <typename T>
inline Eigen::Vector3<T> PTransform<T>::angularInvMul(
    const MotionVec<T>& mv) const {
  using namespace Eigen;
  const Matrix3<T>& E = rotation();
...
template <typename T>
inline Eigen::Vector3<T> PTransform<T>::linearInvMul(
    const MotionVec<T>& mv) const {
  using namespace Eigen;
  const Matrix3<T>& E = rotation();
  const Vector3<T>& r = translation();
  return (E.transpose() * mv.linear() + r.cross(E.transpose() * mv.angular()))
      .eval();
}
...
template <typename T>
inline Eigen::Vector3<T> PTransform<T>::coupleDualMul(
    const ForceVec<T>& fv) const {
  using namespace Eigen;
  const Matrix3<T>& E = rotation();
  const Vector3<T>& r = translation();
...
template <typename T>
inline Eigen::Vector3<T> PTransform<T>::forceDualMul(
    const ForceVec<T>& fv) const {
  using namespace Eigen;
  const Matrix3<T>& E = rotation();
...
template <typename T>
inline Eigen::Vector3<T> PTransform<T>::coupleTransMul(
    const ForceVec<T>& fv) const {
  using namespace Eigen;
  const Matrix3<T>& E = rotation();
  const Vector3<T>& r = translation();

  return (E.transpose() * fv.couple_ + r.cross(E.transpose() * fv.force_))
      .eval();
}

template <typename T>
inline Eigen::Vector3<T> PTransform<T>::forceTransMul(
    const ForceVec<T>& fv) const {
  using namespace Eigen;
  const Matrix3<T>& E = rotation();
...
inline RBInertia<T> PTransform<T>::dualMul(const RBInertia<T>& rbI) const {
  ...
  const Vector3<T>& r = translation();
  Matrix3<T> I;
  I.template triangularView<Lower>() =
      E * (rbI.inertia() +
           vector3ToCrossMatrix(r) * vector3ToCrossMatrix<T>(rbI.momentum()) +
...
inline ABInertia<T> PTransform<T>::dualMul(const ABInertia<T>& rbI) const {
  ...
  I.template triangularView<Lower>() =
      E *
      (rbI.inertia() - rCross * rbI.gInertia().transpose() + (tmpI * rCross)) *
      E.transpose();
...
inline ABInertia<T> PTransform<T>::transMul(const ABInertia<T>& rbI) const {
  ...
  I.template triangularView<Lower>() =
      (E.transpose() * rbI.inertia() * E + rCross * Hp.transpose() -
       (Hp + rCross * Mp) * rCross);
