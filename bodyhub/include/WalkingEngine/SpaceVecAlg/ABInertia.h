...
class ABInertia {
 ...
 public:
   ...
   */
  ABInertia(const TriangularView<matrix3_t, Lower>& ltM, const matrix3_t& H,
            const TriangularView<matrix3_t, Lower>& ltI)
      : M_(ltM), H_(H), I_(ltI) {}

  // Accessor
  /// @return Mass matrix with a zero upper part.
  const matrix3_t& lowerTriangularMassMatrix() const { return M_; }
  ...
  const matrix3_t& gInertia() const { return H_; }
  ...
  ABInertia<T> operator-() const { return ABInertia<T>(-M_, -H_, -I_); }
