...
struct InertialData
{
  ...
  Eigen::Quaterniond orientation2D=Eigen::Quaterniond::Identity(); /** The orientation of the torso represented as a quaternion without the z-Rotation. */
  Eigen::Quaterniond orientation3D=Eigen::Quaterniond::Identity(); /** The orientation of the torso represented as a quaternion including the z-Rotation. */
};
...
private:
  Eigen::Vector3d gyroDeviation=Eigen::Vector3d(0.003*Util::TO_RADIAN, 0.003*Util::TO_RADIAN, 0.003*Util::TO_RADIAN);  //Eigen::Vector3d(0.03*Util::TO_DEGREE, 0.03*Util::TO_DEGREE, 0.03*Util::TO_DEGREE);  // Noise of the gyro in °/s / sqrt(Hz).
  ...
  struct State : public Manifold<3>
  {
    ...
    State(const Eigen::Quaterniond& orientation = Eigen::Quaterniond::Identity()) : orientation(orientation)
    {
  ...
  void estimate(const Eigen::Vector3d& gyro, const Eigen::Vector3d& acc, double timePassed, const Eigen::Vector3d& gyroDeviation, const Eigen::Vector3d& accDeviation);
