...
class LIPMWalk{
	public:
		...
		void SwingAround();
		void YawAround(int YawAmplitude=0, int RollAmplitude=80); //手臂摆动，带默认参数
		...
		Eigen::Vector6d cubicsplineFootPx, cubicsplineFootAx, cubicsplineFootPy, cubicsplineFootAy, cubicsplineFootPz1, cubicsplineFootAz1,cubicsplineFootPz2, cubicsplineFootAz2;
		...
		Eigen::Vector3d convertVecInPToWorld(Eigen::Vector3d vec,Eigen::Vector3d footInWorld);	
		Eigen::Vector3d convertVecInWToP(Eigen::Vector3d vec,Eigen::Vector3d footInWorld);	
		Eigen::Vector2d convertVecInWToPVelocity(Eigen::Vector2d vec,Eigen::Vector3d footInWorld);
		...
		double LArm_Pdf = 0, LArm_Rdf = 80, LArm_elbowdf = 0; //手臂关节角度初始值
		double RArm_Pdf = 0, RArm_Rdf = -80, RArm_elbowdf = 0;  
		double LArm_P = LArm_Pdf, LArm_R = LArm_Rdf, LArm_elbow = LArm_elbowdf; //手臂关节角度
		double RArm_P = RArm_Pdf, RArm_R = RArm_Rdf, RArm_elbow = RArm_elbowdf;  
	...
	// private:
		...
		enum PhaseOfStep
		{
			...
			LeftStance = -1,
			RightStance = 1
		} StepPhase, LastStepPhase,VirtualStepPhase, VirtualLastStepPhase;;
		enum ChangeOfPhase
		{
			NoChange,
			DtoL,
			...
			LtoD,
			RtoD
		} PhaseChange,VirtualPhaseChange;
		void updateStepPhase();

		void updateStepParameter();
		void computerCurrentPosture();

		Eigen::Vector4d observerProcessDeviation = {0.1, 0.1, 3, 3}; /**< The noise of the filtering process that estimates the position of the center of mass */
		Eigen::Vector2d observerMeasurementDeviation = { 2, 2}; /**< The measurement uncertainty of the computed "measured" center of mass position */
		...
		//Yaw角为S坐标系下
		double RobotYaw_Torso,	RobotYaw_Pendulum,	RobotYaw_Present,		RobotYaw_New,	RobotYaw_Old;
		ParameterOfPosture  PosPara, LastPosPara, CurrentPos, LastPos, TouchPos;
		double VirtualRobotYaw_Torso,VirtualRobotYaw_Pendulum,VirtualRobotYaw_Present,VirtualRobotYaw_New,VirtualRobotYaw_Old;
		...
		// void LIPMWalk::TransformTtoP(double Xin, double Yin, double *Xout, double *Yout, double YAWin, double *YAWout);
		void TransformTtoP(double Xin, double Yin, double *Xout, double *Yout, double YAWin, double *YAWout);
		...
		double Rfoot_x_RelaToP, Rfoot_y_RelaToP, Lfoot_x_RelaToP, Lfoot_y_RelaToP, Lfoot_x_RelaToP_Old, Rfoot_x_RelaToP_Old;
		...
		struct CoM_w_frame
		{
			double CoMx[100];
			double CoMy[100];
			double V0_x[100];//not using
			double V0_y[100];//not using
		};// plannedCoM_wF[20]; // 20 steps maxium
		std::vector<CoM_w_frame> plannedCoM_wF;


		ParameterOfPosture  PosPara_wF, LastPosPara_wF, CurrentPos_wF,CurrentPosInP, LastPos_wF, TouchPos_wF,CommandPos_wF,CurrentPosInPtest;//defined in world frame 
		Eigen::Vector6d cubicsplinePyaw_wF, cubicsplineAyaw_wF,cubicsplinePx_wF, cubicsplineAx_wF, cubicsplinePy_wF,cubicsplineAy_wF;// world frame spline
		Eigen::Vector6d cubicsplinePx_InP,cubicsplineAx_InP,cubicsplinePy_InP,cubicsplineAy_InP;
		...
		ParameterOfPosture convertPosPtoW(ParameterOfPosture posInP, Eigen::Vector3d fpInWorld);
		ParameterOfPosture convertPosWtoP(ParameterOfPosture posInP);// auto select footPrint
		ParameterOfPosture convertPosWtoP(ParameterOfPosture posInW, Eigen::Vector3d fpInWorld);
		Eigen::Vector2d simconvertVecInSToWorld(Eigen::Vector2d vec);
		Eigen::Vector6d cubicsplineFootPx_wf, cubicsplineFootAx_wf, cubicsplineFootPy_wf, cubicsplineFootAy_wf, cubicsplineFootPz1_wf, cubicsplineFootAz1_wf,cubicsplineFootPz2_wf, cubicsplineFootAz2_wf,cubicsplineFootPyaw_wf, cubicsplineFootAyaw_wf;
