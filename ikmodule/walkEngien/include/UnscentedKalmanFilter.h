...
struct Manifold
{
  ...
  using Vectord = Eigen::Matrix<double, DOF, 1>; // Handle the element like a vector

  Manifold operator+(const Vectord& vec) const; // The [+] operator
  Manifold& operator+=(const Vectord& vec);
  Vectord operator-(const Manifold& other) const; // The [-] operator
};
...
constexpr V sqr(const V& a) { return a * a; }
  ...
  {
  public:
    using CovarianceType = Eigen::Matrix<double, DOF, DOF>; // The covariance size to use
    template<unsigned N>
    using Vectord = Eigen::Matrix<double, N, 1>; // The vector size to use
  ...
  public:
     ...
     */
    template<unsigned N>
    void update(const Vectord<N>& measurement, std::function<Vectord<N>(const State&)> measurementModel, const Eigen::Matrix<double, N, N>& measurementNoise);

    /**
     * The single dimensional update step to integrate a measurement into an existing hypothesis.
     * In other works this function is referred as measurement step.
     * @param measurement, a double value that represents a measurement
     * @param measurementModel, a function that returns a measurement for a state
     * @param measurementNoise, the measurement specific noise to quantify the uncertainty
     */
    void update(double measurement, std::function<double(const State&)> measurementModel, double measurementNoise);
  ...
  void UnscentedKalmanFilter<State, DOF, Manifold>::predict(std::function<void(State&)> dynamicModel, const CovarianceType& noise)
  {
    ...
    for(State& sigmaPoint : sigmaPoints)
    {
      const Vectord<DOF> dist = sigmaPoint - mean;
  ...
  template<unsigned N>
  void UnscentedKalmanFilter<State, DOF, IsManifold>::update(const Vectord<N>& measurement, std::function<Vectord<N>(const State&)> measurementModel, const Eigen::Matrix<double, N, N>& measurementNoise)
  {
    if(!((measurementNoise.array() >= 0.).all()))
      return;
    if(!(measurementNoise.trace() > 0.))
      return;

    using MeasurementType = Vectord<N>;
    using MeasurementCovarianceType = Eigen::Matrix<double, N, N>;
    using MixedCovarianceType = Eigen::Matrix<double, DOF, N>;
    ...
    for(MeasurementType& Zi : Z)
      z += Zi;
    z /= static_cast<double>(sigmaPoints.size());
   ...
   */
  template<typename State, unsigned DOF, bool IsManifold>
  void UnscentedKalmanFilter<State, DOF, IsManifold>::update(double measurement, std::function<double(const State&)> measurementModel, double measurementNoise)
  {
    if(!(measurementNoise > 0.))
      return;

    using MixedCovarianceType = Eigen::Matrix<double, DOF, 1>;
    ...
    const MixedCovarianceType K = simgaxz * (1. / sigmaz);
  ...
  {
    using Vectord = Eigen::Matrix<double, DOF, 1>; // The vector type to operate on
    ...
    static State run(const Array& sigmaPoints)
    {
      ...
      do
      {
        ...
        for(const State& sigmaPoint : sigmaPoints)
          sum += sigmaPoint - mean;
        mean += sum / static_cast<double>(sigmaPoints.size());
      }
      while(!isZero((lastMean - mean).norm()) && iterations <= 20);
      return mean;
    }
  };
  ...
  struct MeanOfSigmaPoints<State, DOF, Array, false>
  {
    ...
    static State run(const Array& sigmaPoints)
    {
      ...
      for(const State& sigmaPoint : sigmaPoints)
        sum += sigmaPoint;
      return sum / static_cast<double>(sigmaPoints.size());
    }
  };
}
...
using UKF = impl::UnscentedKalmanFilter<Eigen::Matrix<double, DOF, 1>, DOF, false>;
