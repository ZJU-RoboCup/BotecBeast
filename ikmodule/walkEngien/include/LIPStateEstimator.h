...
struct LIPStateEstimatorParameters
{
  ...
  Eigen::Vector2d  positionProcessDeviation = Eigen::Vector2d(0.05,0.05); // Standard deviation of the position in the prediction step (in m per second).
  Eigen::Vector2d  velocityProcessDeviation = Eigen::Vector2d(0.1,0.1); // Standard deviation of the velocity in the prediction step (in m/s per second).
  Eigen::Vector2d  cpProcessDeviation = Eigen::Vector2d(0.05,0.05); // Standard deviation of the zmp in the prediction step (in m per second).
  Eigen::Vector2d  zmpProcessDeviation = Eigen::Vector2d(0.05,0.05); // Standard deviation of the zmp in the prediction step (in m per second).
  Eigen::Vector2d  positionMeasurementDeviation = Eigen::Vector2d(0.01,0.01); // Standard deviation of the zmp in the measurement step (in m per second).
  Eigen::Vector2d  velocityMeasurementDeviation = Eigen::Vector2d(0.01,0.01);
  Eigen::Vector2d  cpMeasurementDeviation = Eigen::Vector2d(0.01,0.01); // Standard deviation of the zmp in the measurement step (in m per second).
  Eigen::Vector2d  zmpMeasurementDeviation = Eigen::Vector2d(0.01,0.01); // Standard deviation of the zmp in the measurement step (in m per second).
};
...
public:
  ...
  struct EstimatedState
  {
    ...
    Eigen::Vector2d zmp = Eigen::Vector2d::Zero(); // relative to current stance foot 
    SupportFoot supportFoot = SupportFoot::right;
    ...
    // EstimatedState predict(double time) const { return EstimatedState(*this).update(time); }
  };

  LIPStateEstimator();
  void init(InertialData& inertialData,Eigen::Vector2d cpinit,Eigen::Vector2d copinit);

  void update(double timePassed, const SupportFoot newSupportFoot,InertialData& inertialData,Eigen::Vector2d cpcmd);
...
private:
  ...
  UKF<8> ukf = UKF<8>(Vector8d::Zero()); // The statevector of the ukf is composed of: position, velocity, zmp;
