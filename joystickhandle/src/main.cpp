...
#define GAIT_X_RANGE (0.07)
#define GAIT_B_RANGE (0.04) // back
#define GAIT_Y_RANGE (0.04)
#define GAIT_A_RANGE (10.0) // alpha角

#define JOY_X_RANGE (1.0)
#define JOY_Y_RANGE (1.0)
#define JOY_A_RANGE (1.0) // alpha角

#define JOY_X_THRESHOLD (0.5)
#define JOY_Y_THRESHOLD (0.5)
#define JOY_A_THRESHOLD (0.5) // alpha角
...
const Eigen::Vector3d GaitCommand = Eigen::Vector3d(0.00, 0.00, 0.0);
uint8_t JoyLockStatus = 1;

void *RosSpin_thread(void *ptr)
{
  while (ros::ok())
  {
    ros::spinOnce();
...
void JoyCallback(const sensor_msgs::Joy::ConstPtr &msg)
{
  ...
  if (JoyLockStatus)
  {
    ...
    if (msg->axes[JOY_L_R_AXIS_LIFE] > JOY_Y_THRESHOLD)
      GaitCommandMsg.data[1] = (msg->axes[JOY_L_R_AXIS_LIFE] - JOY_Y_THRESHOLD) * Y_COEFFICIENT;
    else if (msg->axes[JOY_L_R_AXIS_LIFE] < -JOY_Y_THRESHOLD)
      GaitCommandMsg.data[1] = (msg->axes[JOY_L_R_AXIS_LIFE] + JOY_Y_THRESHOLD) * Y_COEFFICIENT;
    else
      GaitCommandMsg.data[1] = 0.0;

    if (msg->axes[JOY_L_R_AXIS_RIGHT] > JOY_A_THRESHOLD)
      GaitCommandMsg.data[2] = (msg->axes[JOY_L_R_AXIS_RIGHT] - JOY_A_THRESHOLD) * A_COEFFICIENT;
    else if (msg->axes[JOY_L_R_AXIS_RIGHT] < -JOY_A_THRESHOLD)
      GaitCommandMsg.data[2] = (msg->axes[JOY_L_R_AXIS_RIGHT] + JOY_A_THRESHOLD) * A_COEFFICIENT;
    else
...
int main(int argc, char **argv)
{
  ...
  // pthread_create(&thread_command, NULL, RosSpin_thread, NULL);
